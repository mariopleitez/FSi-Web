...
@section('content')
<form action="{{ route('users.update', $user->id) }}" method="PUT" id="userEditar" enctype="multipart/form-data" class="formulario">
    {{ csrf_field() }}
    <div class="form-body">
            ...
            <div class="col-md-12">
                <div class="form-group role_id">
                    ...
                    <select name="role[]"  class="select2 form-control" multiple>
                        ...
                        <option value="{{ $key }}" {{ in_array($key, $my_roles) ? "selected" : "" }}>{{ $value }}</option>
                        @endforeach
                    </select>
                </div>    
            </div>
