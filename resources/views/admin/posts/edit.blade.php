  ...
  <div class="row">
      <div class="col-lg-12">
          <div class="card card-outline-info">
              ...
              <div class="card-body">
                <form role="form" action="{{ route('posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" id="postform">
                  {{ csrf_field() }}
                  {{ method_field('PUT') }}
                    <div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
                        <label for="definpu">Titulo</label>
                        <input type="text" class="form-control" id="definput" placeholder="" name="name" value="{{ old('name', $post->name) }}">
                        @if ($errors->has('name'))
                    ...
                    <div class="row">
                          ...
                          <div class="form-group {{ $errors->has('redactor') ? ' has-error' : '' }}">
                              ...
                              <select name="redactor_id[]" id="" class="form-control select2" required multiple>
                                  @foreach ($redactores as $key => $value)
                                    <option value="{{ $value }}" {{ (in_array($value, $mis_autores)) ? "selected" : "" }}>{{ $key }}</option>
                                  @endforeach
                              </select>
                        ...
                        <div class="col-md-6">
                          <div class="form-group {{ $errors->has('redactor') ? ' has-error' : '' }}">
                              ...
                              <input type="text" name="producto_stripe_id" class="form-control" value="{{ $post->producto_stripe_id }}" required>
                              @if ($errors->has('redactor'))
                                  <span class="help-block">
                                      <strong>{{ $errors->first('redactor') }}</strong>
                                  </span>
                    ...
                    <div class="form-group {{ $errors->has('comment') ? ' has-error' : '' }}">
                      ...
                      <textarea name="comment" class="form-control" id="mymce" cols="30" rows="10">{{ old('comment', $post->comment) }}</textarea>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <select name="posts_type_id" id="" class="form-control">
                                @foreach ($tipos as $key => $value)
                                    <option value="{{ $key }}" {{ ($key == $post->posts_type_id) ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                                ...
                                <div class="input-group date" id="datetimepicker1" data-target-input="nearest">
                                    <input type="text" name="end_date" class="form-control datetimepicker-input" data-target="#datetimepicker1" value="{{ isset($post->end_date) ? \Carbon\Carbon::parse($post->end_date)->format('d/m/Y H:i A') : ''  }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" class="form-control" id="goal"  name="goal" value="{{ $post->goal }}" required>
                            @if ($errors->has('name'))
                                <span class="help-block">
                                    <strong>{{ $errors->first('name') }}</strong>
                                </span>
                    ...
                    <div class="form-group {{ $errors->has('tags') ? ' has-error' : '' }}">
                       ...
                       <select name="tags[]" class="form-control select2multiple" required multiple>
                           @foreach ($tags as $key => $value)
                               <option value="{{ $value }}" {{ (in_array($value, $mis_tags)) ? "selected" : "" }}>{{ $key }}</option>
                           @endforeach
                       </select>
                       @if ($errors->has('tags'))
                           <span class="help-block">
                           ...
                           <div class="form-group">
                               ...
                               <div class="form-check">
                                    <label class="custom-control custom-radio">
                                        <input id="radio1" name="active" type="radio" class="custom-control-input" value="1" {{ ($post->active == 1) ? "checked" : "" }}>
                                        <span class="custom-control-indicator"></span>
                                        <span class="custom-control-description">Activo</span>
                                    </label>
                                    <label class="custom-control custom-radio">
                                        <input id="radio2" name="active" type="radio" class="custom-control-input" value="0" {{ ($post->active == 0) ? "checked" : "" }}>
                                        <span class="custom-control-indicator"></span>
                              ...
                              <div class="form-group {{ $errors->has('country_id') ? ' has-error' : '' }}">
                                  ...
                                  <select name="country_id" class="form-control select2" data-placeholder="Seleccione un pais" id="paisid" required>
                                      ...
                                      @foreach ($paises as $key => $value)
                                         ...
                                         <option value="{{ $value }}" {{ ($value == $post->ciudad->departamento->pais->id) ? "selected" : "" }}>{{ $key }}</option>
                                         @else
                        ...
                        <div class="col-md-4">
                            <div class="form-group {{ $errors->has('state_id') ? ' has-error' : '' }}">
                                  ...
                                  <select name="state_id" class="form-control select2" data-placeholder="Seleccione un departamento" id="departamentoid" required >
                                      ...
                                      @if (isset($post->ciudad->departamento->id))
                                      <option value="{{ $value }}" {{ ($value == $post->ciudad->departamento->id) ? "selected" : "" }}>{{ $key }}</option>
                                      @else
                                      <option value="{{ $value }}">{{ $key }}</option>
                                      @endif
                                      @endforeach
                        ...
                        <div class="col-md-4">
                              <div class="form-group {{ $errors->has('ciudad_id') ? ' has-error' : '' }}">
                                  ...
                                  <select name="city_id" class="form-control select2" data-placeholder="Seleccione una ciudad" id="ciudadid" required>
                                        ...
                                        <option value="{{ $value }}" {{ ($value == $post->ciudad->id) ? "selected" : "" }}>{{ $key }}</option>
                                        @else
                                        <option value="{{ $value }}">{{ $key }}</option>
                                        @endif
                                        @endforeach
                    ...
                    <div class="row">
                     ...
                     <div class="row">
                          <div class="col-md-3">
                              <div class="fileinput fileinput-new" data-provides="fileinput">
                                  <div class="fileinput-new thumbnail" style="width: 200px; height: 150px;">
                                      @if (isset($post->images[0]->image))
                                        <img src="{{ Storage::url($post->images[0]->image) }}" alt="">  
                                      @else
                                  ...
                                  <div style="text-align:center">
                                    ...
                                    <span class="fileinput-exists">Actualizar</span>
                          ...
                          <div class="col-md-3">
                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                  <div class="fileinput-new thumbnail" style="width: 200px; height: 150px;">
                                      @if (isset($post->images[1]->image))
                                        <img src="{{ Storage::url($post->images[1]->image) }}" alt="">  
                                      @else
                                        <img data-src="holder.js/100%x100%" alt="100%x100%" src="data:image/svg+xml;base64," style="height: 100%; width: 100%; display: block;">
                                      @endif
                                  </div>
                          ...
                          <div class="col-md-3">
                              <div class="fileinput fileinput-new" data-provides="fileinput">
                                  <div class="fileinput-new thumbnail" style="width: 200px; height: 150px;">
                                       @if (isset($post->images[2]->image))
                                        <img src="{{ Storage::url($post->images[2]->image) }}" alt="">  
                                      @else
                                        <img data-src="holder.js/100%x100%" alt="100%x100%" src="data:image/svg+xml;base64," style="height: 100%; width: 100%; display: block;">
                                      @endif
                                  </div>
                          ...
                          <div class="col-md-3">
                             <div class="fileinput fileinput-new" data-provides="fileinput">
                                  <div class="fileinput-new thumbnail" style="width: 200px; height: 150px;">
                                      @if (isset($post->images[3]->image))
                                        <img src="{{ asset(str_replace('public','storage', $post->images[3]->image)) }}" alt="">  
                                      @else
                                        <img data-src="holder.js/100%x100%" alt="100%x100%" src="data:image/svg+xml;base64," style="height: 100%; width: 100%; display: block;">
                                      @endif
                                  </div>
                      ...
                      <div class="form-actions" style="text-align: center">
                          <button type="submit" class="btn btn-success col-md-5"> <i class="fa fa-check"></i> EDITAR</button>
...
<script type="text/javascript">
    $(function () {
        $('#datetimepicker1').datetimepicker({
           // defaultDate: '{{  isset($post->end_date) ? Carbon\Carbon::parse($post->end_date)->format("d/m/Y h:mm A") : "" }} '
...
function initMap() {
         ...
         var myLatLng = {lat: {{ isset($post->lat) ? $post->lat : 13.7106262 }},  lng: {{ isset($post->lng) ? $post->lng : -89.2047677 }} };
         var marker =  null;
